...
DECOMPRESSOR_PATH="$REPO_PATH/build/src/NanoLog/src/Nanolog_project/runtime"
NANOLOGFILE="performance-logs"
DECOMP_LOGFILE="decomplog"
LATENCIES_FILE="selected_column.txt"
TODAY=`date '+%Y_%m_%d__%H_%M_%S'`;
DIR_NAME="Performance_stats_$TODAY"
mkdir $DIR_NAME
echo "Decompressing Logs"
cp $REPO_PATH/build/$NANOLOGFILE $DIR_NAME/$NANOLOGFILE
$DECOMPRESSOR_PATH/decompressor decompress $DIR_NAME/$NANOLOGFILE > $DIR_NAME/$DECOMP_LOGFILE

cat $DIR_NAME/$DECOMP_LOGFILE | cut -d '*' -f 2 > $DIR_NAME/$LATENCIES_FILE
...
(awk 'BEGIN {c=0} $1 ~ /^[0-9]/ {c=c+1;} END {print c;}' "$DIR_NAME/$LATENCIES_FILE"; \
        sort -n "$DIR_NAME/$LATENCIES_FILE") | awk '
  BEGIN {
    c = 0;
    sum = 0;
    sum_of_squares = 0;
  ...
  NR==1 {
   ...
   $1 ~ /^[0-9]/  &&  NR!=1 {
    ...
    if (c==0) {min = $1;}
    all[NR] = $1
    if ($1<=6.0) {percentage_for_six_us++;}
  ...
  END {
    ...
    percentage_for_six_us = (percentage_for_six_us/c)*100
    ...
    print "% of packets at 6us:" percentage_for_six_us "%"
    print "Total:" c
...
rm $DIR_NAME/$DECOMP_LOGFILE
cat $DIR_NAME/$OUTPUT_FILE

rm $REPO_PATH/build/$NANOLOGFILE